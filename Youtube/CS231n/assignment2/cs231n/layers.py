...
def conv_forward_naive(x, w, b, conv_param):
    ...
    HH = w.shape[2]
    WW = w.shape[3]
    out_h = int(1 + (x.shape[2] + 2 * p - HH) / s)
    out_w = int(1 + (x.shape[3] + 2 * p - WW) / s)
        ...
        for filt in range(w.shape[0]):
            for ww in range(out_w):
                ...
                for hh in range(out_h):
                    hhh = hh * s
                    out[data, filt, hh, ww] += np.sum(x_pad[data][:, hhh:hhh+HH, www:www+WW] * w[filt])+b[filt]
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x, w, b, conv_param)
    ...
    for data in range(x.shape[0]):
        for filt in range(w.shape[0]):
            for ww in range(out_w):
                ...
                for hh in range(out_h):
                    hhh = hh * s
                    dw[filt] += x_pad[data,:, hhh:hhh+HH, www:www+WW] * dout[data, filt, hh, ww]
                    dx_pad[data,:, hhh:hhh+HH, www:www+WW] += dout[data,filt, hh, ww] * w[filt]
                    
    dx = dx_pad[:,:,p:p+x.shape[2],p:p+x.shape[3]]
...
def max_pool_forward_naive(x, pool_param):
    ...
    s = pool_param['stride']
    h_out = int(1 + (x.shape[2] - pool_height) / s)
    w_out = int(1 + (x.shape[3] - pool_width) / s)
    out = np.zeros((x.shape[0], x.shape[1], h_out, w_out))
    for data in range(x.shape[0]):
        for h in range(h_out):
            for w in range(w_out):
                for channel in range(x.shape[1]):
                    out[data, channel, h, w] = np.max(x[data, channel, h*s:h*s+pool_height, w*s:w*s+pool_width])
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x, pool_param)
...
def max_pool_backward_naive(dout, cache):
    ...
    for data in range(x.shape[0]):
        for h in range(h_out):
            for w in range(w_out):
                for channel in range(x.shape[1]):
                    current = x[data, channel, h*s:h*s+pool_height, w*s:w*s+pool_width]
                    index = np.unravel_index(current.argmax(), current.shape)
                    dx[data,channel][index[0]+h*s, index[1]+w*s] += 1 * dout[data, channel, h, w]
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx
...
def spatial_batchnorm_backward(dout, cache):
    ...
    dx, dgamma, dbeta = batchnorm_backward_alt(y, cache)
